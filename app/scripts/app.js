...
// Inline configuration for jshint below. Prevents `gulp jshint` from failing with quiz starter
// code.
/* jshint unused: false */
...
(function (document) {
  ...
  function addSearchHeader (response) {
    try {
      response = JSON.parse(response).query;  // you'll be moving this line out of here in the next
                                              // quiz!
    } catch (e) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      req.onload = function () {
        if (req.status === 200) {
          ...
          // -> call addSearchHeader
        } else {
          // It failed :(
          // Be nice and reject with req.statusText
          // pass unknown to addSearchHeader
          reject(Error(req.statusText)); //solution
          // reject('unknown');
        }
      };
      req.onerror = function () {
        ...
        reject(Error('Network Error')); //solution
      };
      req.send();
  ...
  window.addEventListener('WebComponentsReady', function () {
      ...
      get('../data/earth-like-results.json')
        ...
        console.log('log error in catch: ', error);
      });
